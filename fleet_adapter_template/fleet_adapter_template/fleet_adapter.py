...
def initialize_fleet(config_yaml, nav_graph_path, node, use_sim_time):
    ...
    # Power systems
    ambient_power_sys = battery.PowerSystem.make(fleet_config['ambient_system']['power'])
    ...
    assert adapter, ("Unable to initialize fleet adapter. Please ensure RMF Schedule Node is running")
    adapter.start()
    time.sleep(1.0)

    node.declare_parameter('server_uri', '')
    server_uri_param = node.get_parameter('server_uri')
    if server_uri_param.type_ == Parameter.Type.NOT_SET:
        node.set_parameters([Parameter('server_uri', Parameter.Type.STRING, '')])
        server_uri = None
    else:
        server_uri = server_uri_param.get_parameter_value().string_value
        if server_uri == "":
            server_uri = None
    ...
    def _updater_inserter(cmd_handle, update_handle):
        ...
        def _action_executor(category: str, description: dict, execution: adpt.robot_update_handle.ActionExecution):
            with cmd_handle._lock:
                if len(description) > 0 and description in cmd_handle.graph.keys:
                    cmd_handle.action_waypoint_index = cmd_handle.find_waypoint(description).index
                else:
        ...
        if cmd_handle.charger_waypoint_index < cmd_handle.graph.num_waypoints:
            cmd_handle.update_handle.set_charger_waypoint(cmd_handle.charger_waypoint_index)
        else:
            cmd_handle.node.get_logger().warn(
                "Invalid waypoint supplied for charger. Using default nearest charger in the map"
            )

    # Initialize robot API for this fleet
    prefix = 'http://' + fleet_config['fleet_manager']['ip'] + ':' + str(fleet_config['fleet_manager']['port'])
    api = RobotAPI(prefix, fleet_config['fleet_manager']['user'], fleet_config['fleet_manager']['password'])
        ...
        while len(missing_robots) > 0:
            ...
            for robot_name in list(missing_robots.keys()):
                ...
                if data['success']:
                    ...
                    if position is None:
                        node.get_logger().info(f'Failed to get initial position of {robot_name}')
                        continue
                    ...
                    if initial_waypoint is not None and initial_orientation is not None:
                        node.get_logger().info(
                            ...
                            f"and orientation [{initial_orientation:.2f}] to initialize starts for robot [{robot_name}]"
                        )
                        # Get the waypoint index for initial_waypoint
                        initial_waypoint_index = nav_graph.find_waypoint(initial_waypoint).index
                        starts = [plan.Start(time_now, initial_waypoint_index, initial_orientation)]
                    else:
                        node.get_logger().info(f"Running compute_plan_starts for robot: {robot_name}")
                        starts = plan.compute_plan_starts(nav_graph, rmf_config['start']['map_name'], position, time_now)

                    if starts is None or len(starts) == 0:
                        node.get_logger().error(f"Unable to determine StartSet for {robot_name}")
                        continue
                    ...
                    if robot.initialized:
                        ...
                        fleet_handle.add_robot(
                            robot, robot_name, profile, [starts[0]], partial(_updater_inserter, robot)
                        )
...
def main(argv=sys.argv):
    ...
    parser.add_argument("-n", "--nav_graph", type=str, required=True, help="Path to the nav_graph for this fleet adapter")
    parser.add_argument("-sim", "--use_sim_time", action="store_true", help='Use sim time, default: false')
