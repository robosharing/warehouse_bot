...
RLCarOdometry::RLCarOdometry() : Node("ackermann_odometry")
{
  ...
  RCLCPP_INFO(get_logger(), "publish_rate : %ld", publish_rate);
  ...
  RCLCPP_INFO(get_logger(), "encoder_resolution : %ld", encoder_resolution);
  ...
  RCLCPP_INFO(get_logger(), "velocity_rolling_window_size : %ld", velocity_rolling_window_size);
  ...
  imu_heading_pub_ = this->create_publisher<Float64>("imu_heading", rclcpp::QoS(10));

  joint_state_sub_ = this->create_subscription<JointState>("joint_states", rclcpp::QoS(10).best_effort(),
                                                           std::bind(&RLCarOdometry::jointstateCallback, this, std::placeholders::_1));

  cmd_vel_sub_ = this->create_subscription<Twist>("cmd_vel", rclcpp::QoS(10),
  ...
  if (has_imu_heading_ == true)
    ...
    steering_angle_sub_ = this->create_subscription<Float64>(
        "steering_angle_middle", rclcpp::QoS(10).best_effort(),
