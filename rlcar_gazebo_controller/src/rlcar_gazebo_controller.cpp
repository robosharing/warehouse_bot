...
private:
  ...
  void publish() {
    ...
    double wheel_turning_speed_left_rear_wheel;
    double wheel_turning_speed_right_rear_wheel;
    double wheel_turning_speed_front_wheel;

    double wheel_turning_speed_com;
    double alfa_front_wheel;
    ...
    if (steering_radius >= 0) {
        ...
        wheel_turning_speed_right_rear_wheel = limit_wheel_speed(vel_right_rear_wheel / wheel_radius);
        ...
        wheel_turning_speed_left_rear_wheel = limit_wheel_speed(vel_left_rear_wheel / wheel_radius);

        auto vel_com = omega_turning_speed * steering_radius;
        wheel_turning_speed_com = limit_wheel_speed(vel_com / wheel_radius);
    } else {
        wheel_turning_speed_right_rear_wheel = limit_wheel_speed(linear_velocity / wheel_radius);
        wheel_turning_speed_left_rear_wheel = limit_wheel_speed(linear_velocity / wheel_radius); 
        wheel_turning_speed_com = limit_wheel_speed(linear_velocity / wheel_radius);
    }

    if (steering_radius >= 0 && linear_velocity != 0) {
        auto distance_to_turning_point_front_wheel = sqrt(
            pow(L, 2) + pow(steering_radius, 2)
        );

        wheel_turning_speed_front_wheel = limit_wheel_speed(
    ...
    auto front_steering = -1 * turning_sign * linear_sign * alfa_front_wheel;
    steering_msg.data.push_back(front_steering);

    throttling_msg.data.push_back(wheel_turning_speed_left_rear_wheel);
    throttling_msg.data.push_back(wheel_turning_speed_right_rear_wheel);
    throttling_msg.data.push_back(wheel_turning_speed_front_wheel);

    steering_msg_middle.data = round_digit(front_steering, 5);
    throttling_msg_middle.data = wheel_turning_speed_com;
