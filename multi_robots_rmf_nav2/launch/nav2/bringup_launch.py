...
def generate_launch_description():
    ...
    warehouse_dir = get_package_share_directory("multi_robots_rmf_nav2")
    nav2_launch_dir = os.path.join(bringup_dir, "launch")
    warehouse_launch_dir = os.path.join(warehouse_dir, "launch", "nav2")
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        "params_file", default_value=os.path.join(warehouse_dir, "config", "nav2_params.yaml"), description="Full path to the ROS2 parameters file to use for all launched nodes"
    )
    ...
    bringup_cmd_group = GroupAction(
        [
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(warehouse_launch_dir, "slam_launch.py")),
                condition=IfCondition(slam),
                launch_arguments={"namespace": namespace, "use_sim_time": use_sim_time, "autostart": autostart, "params_file": params_file}.items(),
            ),
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(os.path.join(warehouse_launch_dir, "localization_launch.py")),
